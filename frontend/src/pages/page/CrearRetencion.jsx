...
import { createRetention } from '../../services/retentionService';
...
const CrearRetencion = () => {
  ...
  useEffect(() => {
    const fetchInvoice = async () => {
      try {
        ...
        // Verificar si la factura es válida para crear retención
        if (!response || response.status === 'cancelled') {
          setError('No se puede crear una retención para esta factura');
          setLoading(false);
          return;
        }
        ...
        if (response.data && response.data.items && response.data.items.length > 0) {
          const subtotal = response.data.subtotal || 
            response.data.items.reduce((sum, item) => sum + (item.price * item.quantity), 0);
        ...
        setError('Error al cargar la factura: ' + (err.message || 'Error desconocido'));
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await createRetention(retention);
      
      Swal.fire({
        title: 'Retención creada',
        text: 'La retención ha sido creada exitosamente',
        icon: 'success',
        confirmButtonText: 'Ver retenciones'
      }).then(() => {
        navigate('/dashboard/retenciones');
      });
    } catch (err) {
      console.error('Error creando retención:', err);
      setError('Error al crear la retención: ' + (err.message || 'Error desconocido'));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    >
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        <div className="bg-gray-50 p-4 border-b border-gray-200 flex justify-between items-center">
          <h1 className="text-2xl font-bold">Nueva Retención</h1>
          <Button
            color="secondary"
            size="sm"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                <div className="bg-gray-50 p-4 rounded-lg border">
                  <h2 className="text-lg font-semibold mb-3">Factura</h2>
                  <p className="mb-2"><strong>Número:</strong> {invoice?.receiptNumber || invoice?.invoiceNumber}</p>
                  <p className="mb-2"><strong>Fecha:</strong> {invoice ? new Date(invoice.dateTime || invoice.date).toLocaleDateString() : ''}</p>
                  <p className="mb-2"><strong>Cliente:</strong> {invoice?.customer?.name}</p>
                  <p className="mb-2"><strong>RUC/CI:</strong> {invoice?.customer?.identification}</p>
                  <p className="mb-2"><strong>Subtotal:</strong> ${invoice?.subtotal?.toFixed(2) || '0.00'}</p>
                  <p className="mb-2"><strong>IVA:</strong> ${invoice?.tax?.toFixed(2) || '0.00'}</p>
                  <p className="mb-2"><strong>Total:</strong> ${invoice?.total?.toFixed(2) || '0.00'}</p>
                </div>
              </div>
            ...
            <div className="flex justify-between">
              ...
              >
                {submitting ? (
                  ...
                  <>
                    <FaSave className="mr-2" /> Guardar Retención
                  </>
                )}
              </Button>
            </div>
